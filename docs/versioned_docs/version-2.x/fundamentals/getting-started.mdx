...
Yes! Hermes enables the lazy bundle format (`mmap`) by default, but you still need a way to manage preloading and caching. This library provides that abstraction layer. See [enabling lazy bundle](./enabling-lazy-bundle.md) for more details on Hermes vs JSC.
